...
//
// V2 slice 1: implementations live in providers/anthropic/billing.js, loaded
// via providers/index.js. The wrapper names and signatures below are
// preserved so call sites elsewhere in this file (and module.exports) do not
// change. Future slices add OpenAI/Codex by registering additional plugins;
// audit.js stays plugin-agnostic.
const { loadProvider } = require("./providers");

async function fetchAnthropicUsage(apiKey, days = 30) {
  return loadProvider("anthropic").fetchUsage(apiKey, days);
}

async function countTokens(apiKey, opts) {
  return loadProvider("anthropic").countTokens(apiKey, opts);
}

async function fetchAnthropicCostReport(adminKey, days = 30) {
  return loadProvider("anthropic").fetchCostReport(adminKey, days);
}
